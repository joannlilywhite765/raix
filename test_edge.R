...
air_configure(backend = "openai", api_key = "")
...
r <- tryCatch({air_configure(backend="ollama", base_url="http://127.0.0.1:1"); air_send(long)}, error = function(e) "CAUGHT")
...
for (i in 1:10) tryCatch(air_configure(backend = sample(c("openai","ollama","claude","deepseek","kimi","zai"), 1)), error = function(e) NULL)
ok <- ok + 1; cat("OK\n")
